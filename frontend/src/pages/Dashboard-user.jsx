...
export default function DashboardUser() {
  ...
  // Låne et produkt
  const lånProdukt = async (produkt_id) => {
    const bruker_id = prompt("Skriv inn ditt ID eller brukernavn:");
    const retur_dato = prompt("Til hvilken dato låner du produktet (YYYY-MM-DD)?");

    if (!bruker_id || !retur_dato) return;

    try {
      await fetch(`/items/${produkt_id}/take`, {
        method: "POST",
        credentials: "include",
      ...
      setProdukter(prev =>
        prev.map(p => p.id === produkt_id ? { ...p, quantity: p.quantity - 1 } : p)
      );
    } catch (err) {
  ...
  const leverProdukt = async (produkt_id) => {
    const bruker_id = prompt("Skriv inn ditt ID eller brukernavn:");

    if (!bruker_id) return;

    try {
      await fetch(`/items/${produkt_id}/return`, {
        method: "POST",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className="p-6">
      ...
      <h2 className="text-xl mb-4">Tilgjengelige produkter</h2>
